...
| Model | Fields |
|---|---|
| `RequesterUser` | `id` (UUID), `fullName`, `email` (unique), `isActive`, `createdAt`, `updatedAt` |
| `Category` | `id` (integer, autoincrement), `name` (unique), `isActive`, `createdAt`, `updatedAt` |
| `RelatedSystem` | `id` (integer, autoincrement), `name` (unique), `isActive`, `createdAt`, `updatedAt` |
| `Ticket` | `id` (UUID), `ticketNumber` (unique), `requesterId`, `categoryId`, `relatedSystemId`, `summary`, `description`, `requestedPriority`, `currentStatus`, `createdAt`, `updatedAt` |
| `Attachment` | `id` (UUID), `ticketId`, `originalFilename`, `storedFilename`, `mimeType`, `sizeBytes`, `uploadedById`, `uploadedAt`, `removedAt`, `removedById`, `removedReason` |
...
| DEC-04 | Reference tables (`Category`, `RelatedSystem`) keep integer primary keys; ownership-bearing entities (`RequesterUser`, `Ticket`, `Attachment`) use UUIDs | The System-Level SDS specifies UUID identifiers for primary entities. That reasoning applies to entities an attacker could enumerate: a sequential `Ticket` id would let one Requester reach another's ticket by editing a number in the URL, which BR-18 exists to prevent. Reference data is public, carries no ownership, and gains nothing from an unguessable key. Keeping `Category` on its existing integer key also avoids a destructive migration of Lab 1 data and keeps the Lab 1 test suite passing, which DoD Part 1 requires. Recorded as a partial deviation from the SDS identifier convention. |
...
| DEC-08 | Client route paths are fixed as `/tickets`, `/tickets/new`, and `/lab-01`, with `/` redirecting to `/tickets` | No document in this sprint names a client URL: Section 6 and `ui-spec.md` name screens and navigation labels only. Leaving the vocabulary unstated would have each of Issues #16, #17, #18 and test E2E-03 invent its own. Fixing it once here keeps ticket detail (`/tickets/{ticketId}`, added in #18) consistent with the rest. |
...
| A-03 | Ticket-number allocation is safe under concurrent creation because the increment is a single atomic database statement, not because contention is low | Superseded assumption. This originally read that concurrency was low enough that a transactional sequence table would not become a contention point, which quietly made correctness depend on load. It did not hold: the first implementation computed the successor in application code from a stale read, and eight parallel creations failed six times (API-41). The allocation now uses an atomic `increment`, so the row lock serialises callers and each receives a distinct value. Contention affects throughput, not correctness. |
| A-04 | The Lab 1 route `GET /api/categories` is retained unchanged alongside the new `GET /api/v1/categories` | Lab 1 evidence and its test suite continue to pass without modification; the unversioned route is retired in a later sprint once nothing depends on it. |
| A-05 | The Lab 1 page is retained at `/lab-01`, rendered outside the application shell | A-04 preserves the Lab 1 API route; this extends the same intent to the Lab 1 screen, which stays reachable and visually unchanged. Two consequences are accepted deliberately: it carries no development notice, although `ui-spec.md` Section 4 asks for one on every screen, and it uses Bootstrap colours that are absent from the `ui-spec.md` Section 1.1 token table. It is therefore **excluded from AC-44 and STY-11**, which apply to the five Lab 2 screens in Section 6. Wrapping it in the Zen Green shell to satisfy either would change the Lab 1 slice, which is the thing being preserved. |
| A-06 | Until Issue #18 delivers `POST /api/v1/tickets/{ticketId}/attachments`, the Create Ticket screen offers no attachment selection at all, and states that attachments are added from Ticket Detail after creation | `api-spec.md` Section 7 makes creation and upload separate calls, so Lab 2 passes through a state where the first exists and the second does not. Accepting files the screen cannot send would let a Requester believe evidence was attached when nothing was transmitted, which is worse than offering nothing. The selection-time validation rule required by AC-17 is implemented and tested (UI-30); only the entry point is withheld. **A temporary constraint, removed by #18**, along with the disabled View Ticket action on the success panel, which cannot work until `/tickets/{ticketId}` is served. |
