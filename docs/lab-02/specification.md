...
| A-06 | **Retired by #18.** While Issue #18 was outstanding, the Create Ticket screen offered no attachment selection and its View Ticket action was a disabled button, because `POST /api/v1/tickets/{ticketId}/attachments` and `/tickets/{ticketId}` did not yet exist. Accepting files the screen could not send, or offering a link that redirected to the list, would each have told the Requester something untrue. Both constraints are lifted: selection is available again with the partial-success panel AC-38 requires, and View Ticket links to the served route. Recorded rather than deleted, because the constraint governed the behaviour of two merged commits. |
