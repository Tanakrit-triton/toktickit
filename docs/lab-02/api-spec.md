...
Section 8 of `specification.md` lists the status codes for this sprint. Two codes were added while writing this contract and must be reflected there before the documentation PR is merged:

- `403 REQUESTER_INACTIVE` — inactive Requester in header (BR-13 had no status assigned)
- `410 ATTACHMENT_REMOVED` — download of a removed attachment, previously conflated with `404`

`410` is used rather than `404` because the resource is known to have existed and its metadata is still visible to this same Requester in the attachment list. Reporting it as never-existing would contradict what the UI is displaying.
